...
###########################
```
Defining the server function is like putting a brain in the app. Now it can receive inputs and tell the app what to do. The workhorses of your apps server side code are rendering functions, you will be assigning them to outputs that appear in your user interface. Each `render{Type}` function is designed to produce a particular type of output (e.g. text, tables, and plots), and is often paired with a `{type}Output function`. For example, in the app we're building in this book, `renderPrint()` is paired with `verbatimTextOutput()` to display a statistical summary with fixed-width (verbatim) text, and `renderTable()` is paired with `tableOutput()` to output the data in a table.
