...
2. Go to the RStudio Cloud class project for this session https://rstudio.cloud/project/4266255
...
First we need to import the population data^[population data retrieved from the FRED, the Federal Reserve Bank of St. Louis Economic Data, https://fred.stlouisfed.org/release/tables?rid=118&eid=259194]. In addition to importing a file on your computer, you can also import data using a url to download data from the internet directly to your R session.
...
1. Be sure the `tidyverse` is loaded in your R session. 
2. Use `read_csv()` and the url "https://raw.githubusercontent.com/swcarpentry/r-novice-gapminder/main/data/gapminder_data.csv" to import this [gapminder](https://www.gapminder.org/data/) data on life expectancy. Assign to an object called `gapminder`.
3. Use `summary()` and `glimpse()` to explore the data. How many observations and variables are there? What data types are present?
4. Use `distinct()` and `count()` to explore the values present in the `country`, `year`, and `continent` variables.
5. Use `filter()` to select all rows in `gapminder` with a country of your choice.
6. Use `filter()` to select all rows with a year of your choice.
7. Use `filter()` to select all rows with a continent and year of your choice.
8. Use `filter()` to select all rows with a greater than mean `gdpPercap`. 
9. Use `group_by` and `summarize` to create a new object with mean life expectancy per continent.
10. Use `arrange()` to sort the object you created in question 9 in descending order. 