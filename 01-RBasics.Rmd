...
```{r}
sort(table(subset_variants$ALT), decreasing = TRUE)
```

We can create a object `snps_variants` using a new type of subsetting with `%in%` that keeps rows corresponding to a vector of values in our case the values "A", "C", "G", "T".
...
If you want to do anything beyond very simple plots, though, it’s generally better to switch to ggplot2 which you will learn in Session 3 of the CDABS summeR workshops.
...
11. Use the `library()` function to load the package called `datasets`. This package contains a number of example datasets you can use for practice. 
