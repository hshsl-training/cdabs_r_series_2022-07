...
Let's do some calculations with the population of Maryland ^[Population data from https://www.census.gov/quickfacts/MD]. First we can
...
Census Bureau ^[Population data from https://www.census.gov/quickfacts/MD], 78% of the Maryland population is over 18. We can use
...
Next, we want to know what percentage of people in Maryland have been fully vaccinated for COVID-19. As of early July 2022 4.68 million people of all ages have been vaccinated in
Maryland ^[Vaccination information from https://coronavirus.maryland.gov/#Vaccine]. So let's create another object:
...
Data frames are another VIP data structure in R. While vectors are one dimensional data structures. Data frames are two
...
Data frames are usually created by reading in a dataset using the `read.table()` or `read.csv()` which we will cover in our next session. However, data frames can also be created explicitly with the `data.frame()` function or they can be coerced from other types of objects like lists.
Now that we know vectors form the columns of data frames, we can take the vectors we created for our patient data and combine them together in a data
frame.
...
Lets look at the first few rows of `alt_alleles` using `head()`
```{r}
head(alt_alleles)
```
